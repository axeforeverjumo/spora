...
must_haves:
  truths:
    ...
    - "System blocks saving task when segment does not belong to project's sale order (ValidationError)"
  ...
  artifacts:
    - path: "addons/spora_segment/models/project_task.py"
      provides: "project.task extension with segment_id and cross-order validation"
...
<task type="auto">
  ...
  <action>
    **1. Create `addons/spora_segment/models/project_task.py`:**
    ...
    Add cross-order validation constraint:
    ```python
    @api.constrains('segment_id', 'project_id')
    def _check_segment_order(self):
        """Validate segment belongs to project's sale order."""
        for task in self:
            if not task.segment_id or not task.project_id or not task.project_id.sale_order_id:
                continue
            if task.segment_id.order_id != task.project_id.sale_order_id:
                raise ValidationError(
                    'El segmento "%s" no pertenece al presupuesto de este proyecto. '
                    'Por favor, seleccione un segmento del presupuesto correcto.'
                    % task.segment_id.display_name
                )
    ```

    Import ValidationError from odoo.exceptions.
  ...
  <done>
    - project_task.py exists with segment_id Many2one + _check_segment_order constraint
...
6. **Constraint fires:** Creating task with segment from wrong order raises ValidationError
</verification>
...
- Cross-order validation raises ValidationError
