...
must_haves:
  truths:
    ...
    - "Test confirms cross-order segment assignment raises ValidationError"
...
<task type="auto">
  ...
  <action>
    ...
    **Test methods — PROJ requirements:**
    ...
    `test_cross_order_segment_blocked` (PROJ-03):
    - Create task in project1 with segment from order2 → assertRaises(ValidationError)

    `test_segment_change_to_wrong_order_blocked` (PROJ-03):
    - Create task with valid segment, then write segment_id from wrong order → assertRaises(ValidationError)

    `test_segment_on_project_without_sale_order` (PROJ-03):
    - Create task in project_manual with segment_id → should NOT raise (no sale_order_id on project means no cross-validation triggers; constraint only checks if both segment_id AND project.sale_order_id exist)
  ...
  <done>
    - test_project_task_segment.py exists with ~18 test methods covering all PROJ and SEC requirements
    - Tests reference correct models, fields, and exception types
...
<task type="auto">
  ...
  <action>
    ...
    **4. Fix any failures:**
    ...
    - ValidationError not raised: check constraint decorator field list, check field values in test data
    ...
    **5. Verify all 3 test suites pass with 0 failures:**
    ...
    Expected: ~26 (Phase 1) + ~18 (Phase 2) + ~18 (Phase 3) = ~62 total tests, all passing.
  </action>
  <verify>
    ...
    - test_project_task_segment.py: ~18 tests OK
...
- PROJ-03 (cross-order validation): 3 tests (blocked, change blocked, no-order skip)
...
<success_criteria>
- All ~62 tests pass (Phase 1 + Phase 2 + Phase 3) with 0 failures and 0 errors
- Every PROJ and SEC requirement has at least one test validating it
- Tests cover both positive cases (valid operations succeed) and negative cases (invalid operations blocked)
