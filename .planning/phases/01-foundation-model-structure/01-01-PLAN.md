...
<task type="auto">
  ...
  <action>
    ...
    4. Create `addons/spora_segment/models/sale_order_segment.py`:
       ...
       - description: Text (optional, for segment notes/description)
       ...
       - @api.constrains('parent_id') method _check_hierarchy():
         Step 1: Call self._has_cycle() -- Odoo 18 method for circular reference detection
                 NOTE: Use _has_cycle() NOT _check_recursion() (deprecated in Odoo 18)
                 Semantics: _has_cycle() returns True if cycle exists (inverse of old _check_recursion)
         Step 2: For each segment, walk parent_id chain counting depth. If depth > 4, raise ValidationError.
         Step 3: Walk DOWN to check max depth of entire subtree (prevents reparenting violations).
                 Use helper method _get_max_descendant_depth() to recursively check children.
       ...
       Helper method:
       - _get_max_descendant_depth(segment): Recursively returns max depth of descendants below segment.
         Returns 0 if no children, otherwise 1 + max(child depths).
    ...
    - Do NOT use _check_recursion() -- use _has_cycle() (Odoo 18 standard, semantics inverted)
    ...
    - Do NOT forget to validate subtree depth when reparenting (walk descendants, not just parent)
  ...
  <verify>
    ...
    3. Model file contains: _parent_store = True, parent_path field, _has_cycle(), MAX_HIERARCHY_DEPTH, _get_max_descendant_depth
    4. Model file does NOT contain deprecated _check_recursion()
    5. Security CSV has exactly 2 data rows (user + manager)
    6. Manifest data list has security BEFORE views (Odoo loads in order, security must come first)
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    ...
    2. Form View (sale_order_segment_view_form):
       ...
       - Group 2: sequence, level (add readonly="1" explicitly to level field for clarity)
